...
def download_data_from_bigquery(query: str, billing_project_id: str) -> pd.DataFrame:
    ...
    # while not job.done():
    #     sleep(1)
    print("Getting result from query")
    results = job.result()
    print("Converting result to pandas dataframe")
    dfr = results.to_dataframe()
    print("End download data from bigquery")
    return dfr
...
def get_dataset_processor_info(api, processor_name: str):
    ...
    """
    print(f"Getting dataset processor info for {processor_name}")
    dataset_processors_response = api.get(
        path="dataset_processors",
    )

    # print(dataset_processors_response)
...
def query_data_from_gcp(  # pylint: disable=too-many-arguments
    ...
    """
    print(f"Start downloading {dataset_id}.{table_id} data")
    ...
    print(f"{table_id} data saved on {savepath}")
    return savepath
