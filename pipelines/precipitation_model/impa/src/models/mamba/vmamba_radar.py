...
# except:
#     from csm_triton import CrossMergeTriton, CrossScanTriton, CrossScanTriton1b1
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            index = i_layer + 1 if (i_layer < self.num_layers - 1) else i_layer
    ...
    def _make_unpatch(
        ...
        return nn.Sequential(
            ...
            (
                nn.ConvTranspose2d(
                    embed_dim, in_chans, kernel_size=patch_size, stride=patch_size, bias=True
                )
            ),
    ...
    def _make_upsample(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False):
        ...
        return nn.Sequential(
            (nn.Identity() if channel_first else Permute(0, 3, 1, 2)),
            nn.ConvTranspose2d(out_dim, dim, kernel_size=2, stride=2),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),
            norm_layer(dim),
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            x = layer(x)
        for out_layer in self.out_layers[::-1]:
            x = out_layer(x)
        # x = self.classifier(x)
        x = self.unpatch(x)
        return x
